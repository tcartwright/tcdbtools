...
function ShrinkFile($SqlCmdArguments, [string] $fileName, [int]$size, [int]$targetSizeMB = 5, [int]$timeout, [int]$ShrinkIncrementMB = 0) {
    ...
    if ($size -gt 50 -or $size -gt $targetSize) {
        if ($shrinkIncrement -gt 0) {
            for($x = $size; $x -gt $targetSize; $x -= $shrinkIncrement) {
                $size = $x
                $sql = $rawsql -f $x
                Write-Host "[$($sw.Elapsed.ToString($swFormat))] PERFORMING: $sql" -ForegroundColor Green
                Write-Verbose $sql
    ...
    Write-Host "[$($sw.Elapsed.ToString($swFormat))] PERFORMING FINAL SHRINK: $sql" -ForegroundColor Green
    Write-Verbose $sql
